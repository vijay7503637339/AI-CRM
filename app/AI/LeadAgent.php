...
final class LeadAgent
{
    ...
    private function baseline(array $lead): array
    {
        ...
        if ((float)($lead['value'] ?? 0) >= 100000) { $factors[] = 'High opportunity value'; }
        if (!empty($lead['follow_up_at'])) { $factors[] = 'Follow-up scheduled'; }
    ...
    private function callOpenAI(string $apiKey, array $lead, array $activities, array $baseline): ?array
    {
        ...
        if (!is_array($data) || !isset($data['score'], $data['qualification'], $data['summary'], $data['next_action'])) return null;
