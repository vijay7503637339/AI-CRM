...
if ($q !== '') {
    ...
    $like="%{$q}%"; $stmt->execute([$like,$like,$like,$like]); $leads=$stmt->fetchAll();
} else { $leads=db()->query("SELECT l.*, s.name AS stage_name FROM leads l JOIN pipeline_stages s ON s.id=l.stage_id ORDER BY l.created_at DESC")->fetchAll(); }
?>
...
<section class="panel"><form class="search" method="get"><input name="q" value="<?=e($q)?>" placeholder="Search name, company, email or phone"><button class="btn" type="submit">Search</button></form><div class="table-wrap"><table><thead><tr><th>Name</th><th>Contact</th><th>Source</th><th>Stage</th><th>Value</th><th>AI</th><th>Follow-up</th></tr></thead><tbody><?php if(!$leads):?><tr><td colspan="7" class="empty">No matching leads.</td></tr><?php endif;?><?php foreach($leads as $lead):?><tr><td><a href="lead-view.php?id=<?=$lead['id']?>"><strong><?=e($lead['name'])?></strong></a><br><span class="muted small"><?=e($lead['company'])?></span></td><td><?=e($lead['phone'])?><br><span class="muted small"><?=e($lead['email'])?></span></td><td><?=e($lead['source'])?:'—'?></td><td><span class="pill"><?=e($lead['stage_name'])?></span></td><td>₹<?=number_format((float)$lead['estimated_value'],0)?></td><td><?= $lead['ai_score']!==null ? (int)$lead['ai_score'].'/100':'Pending' ?></td><td><?= $lead['next_follow_up'] ? e(date('d M Y, h:i A',strtotime($lead['next_follow_up']))) : '—' ?></td></tr><?php endforeach;?></tbody></table></div></section></main></body></html>
