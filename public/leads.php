...
if ($q !== '') {
    $stmt = db()->prepare("SELECT * FROM leads WHERE name LIKE ? OR company LIKE ? OR email LIKE ? OR phone LIKE ? ORDER BY created_at DESC");
...
<section class="panel"><form class="search" method="get"><input name="q" value="<?=e($q)?>" placeholder="Search name, company, email or phone"><button class="btn" type="submit">Search</button></form><div class="table-wrap"><table><thead><tr><th>Name</th><th>Contact</th><th>Source</th><th>Status</th><th>Value</th><th>AI</th><th>Follow-up</th></tr></thead><tbody><?php if(!$leads):?><tr><td colspan="7" class="empty">No matching leads.</td></tr><?php endif;?><?php foreach($leads as $lead):?><tr><td><a href="lead-view.php?id=<?=$lead['id']?>"><strong><?=e($lead['name'])?></strong></a><br><span class="muted small"><?=e($lead['company'])?></span></td><td><?=e($lead['phone'])?><br><span class="muted small"><?=e($lead['email'])?></span></td><td><?=e($lead['source'])?:'—'?></td><td><span class="pill"><?=e($statusLabels[$lead['status']] ?? ucfirst($lead['status']))?></span></td><td>₹<?=number_format((float)$lead['value'],0)?></td><td><?= $lead['ai_score']!==null ? (int)$lead['ai_score'].'/100':'Pending' ?></td><td><?= $lead['follow_up_at'] ? e(date('d M Y, h:i A',strtotime($lead['follow_up_at']))) : '—' ?></td></tr><?php endforeach;?></tbody></table></div></section></main></body></html>
